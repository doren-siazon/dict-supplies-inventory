<?php
	...
	if(isset($_POST['returnDetailsreturnID'])){
		...
		$returnDetailsreturnID = htmlentities($_POST['returnDetailsreturnID']);
		
		$quantityInOriginalOrder = 0;
		$quantityInNewOrder = 0;
		$originalStockInItemTable = 0;
		$newStock = 0;
		$originalreturnItemNumber = '';
		
		// Check if mandatory fields are not empty
		if(isset($returnDetailsItemNumber) && isset($returnDetailsreturnDate) && isset($returnDetailsQuantity)){
			...
			// Check if returnID is empty
			if($returnDetailsreturnID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a return ID.</div>';
				exit();
			}
			...
			// Get the quantity and itemNumber in original return order
			$orginalreturnQuantitySql = 'SELECT * FROM returnitem WHERE returnID = :returnID';
			$originalreturnQuantityStatement = $conn->prepare($orginalreturnQuantitySql);
			$originalreturnQuantityStatement->execute(['returnID' => $returnDetailsreturnID]);
						
			if($originalreturnQuantityStatement->rowCount() > 0){
				
				// return details exist in DB. Hence proceed to calculate the stock
				$originalQtyRow = $originalreturnQuantityStatement->fetch(PDO::FETCH_ASSOC);
				...
				if($originalOrderItemNumber !== $returnDetailsItemNumber) {
					...
					$newItemCurrentStockStatement->execute(['itemNumber' => $returnDetailsItemNumber]);
					...
					$enteredQuantityForNewItem = $returnDetailsQuantity;
					...
					$newItemStockUpdateStatement->execute(['stock' => $newItemNewStock, 'itemNumber' => $returnDetailsItemNumber]);
					...
					// Finally UPDATE the return table for new item
					$updatereturnDetailsSql = 'UPDATE returnitem SET itemNumber = :itemNumber, returnDate = :returnDate, categoryName = :categoryName, itemName = :itemName, description = :description, quantity = :quantity WHERE returnID = :returnID';
					$updatereturnDetailsStatement = $conn->prepare($updatereturnDetailsSql);
					$updatereturnDetailsStatement->execute(['itemNumber' => $returnDetailsItemNumber, 'returnDate' => $returnDetailsreturnDate, 'categoryName' => $returnDetailscategoryName, 'itemName' => $returnDetailsItemName, 'description' => $returnDetailsDescription, 'quantity' => $returnDetailsQuantity, 'returnID' => $returnDetailsreturnID]);
					...
					if($stockStatement->rowCount() > 0){
						...
						// Item exists in the item table, therefore, start inserting data to return table	
						// Calculate the new stock value using the existing stock in item table
						$row = $stockStatement->fetch(PDO::FETCH_ASSOC);
						$quantityInNewOrder = $returnDetailsQuantity;
						...
						// Next, update the return table
						$updatereturnDetailsSql = 'UPDATE returnitem SET returnDate = :returnDate, description = :description, quantity = :quantity WHERE returnID = :returnID';
						$updatereturnDetailsStatement = $conn->prepare($updatereturnDetailsSql);
						$updatereturnDetailsStatement->execute(['returnDate' => $returnDetailsreturnDate, 'description' => $returnDetailsDescription, 'quantity' => $returnDetailsQuantity, 'returnID' => $returnDetailsreturnID]);
				...
				// returnID does not exist in return table, therefore, you can't update it 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>The return item  does not exist in DB for the given return ID. Therefore, can\'t update.</div>';
				exit();
